...
console.log(movies[0].title); // object inside array so when we access array we use [] sign.
...
console.log(movies[1].genres[movies.length-1]);
// 18. Log all the genres of the first movie one by one
console.log(movies[1].genres[movies.length]);
...
/* -first go to array-Name and execute.
   - then go to actors and execute first index using []
   - using if else condition to check first movies(0) index-1 is equal to that expression or not 
   - if it match then log true 
   -if missmatch then log false
   */
// 22. Log if the year of all three movies is greater than `2017` or not one by one. (Log true or false)
if(movies[0],movies[1],movies[2].year>2017){
  console.log("true");
}else{
  console.log("false");
}
...
console.log(movies[0],movies[1],movies[2].title);
